...
import { numberSlot, slotList, item, selected, valueView } from "./SlotMachineInput.css.ts";
    ...
    useLayoutEffect(() => {
        ...
        if (showPortal && isExpanded && !entranceDoneRef.current) {
            void animate(
                translateY,
                [
                    calculateSnapPosition(itemHeight, selectedIndex) - SNAP_OFFSET,
                    calculateSnapPosition(itemHeight, selectedIndex),
                ],
            ...
            {isExpanded && (
                <motion.div
                    ...
                    animate={{
                        ...
                        top: (portalPosition?.top || 0) - SNAP_OFFSET,
                    }}
                    exit={{
                        height: COLLAPSED_HEIGHT,
                        top: portalPosition?.top || 0,
                ...
                >
                    <motion.div
                        ...
                        style={{ y: translateY }}
                        exit={{
                            y: calculateSnapPosition(itemHeight, selectedIndex) - SNAP_OFFSET,
                        }}
                        transition={{ duration: ANIMATION_DURATION }}
                        dragConstraints={{
                            bottom: -itemHeight / DRAG_CONSTRAINT_DIVISOR,
                            top:
                                -(itemHeight / DRAG_CONSTRAINT_DIVISOR) -
                                itemHeight * (numbers.length - 1),
                        }}
                        onDrag={onDrag}
